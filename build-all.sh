...
if [ -z "$*" ]
then
  MVN_ARGS="-Pscala-2.12.x -Peclipse-photon $ADDITIONAL_MVN_OPTS clean install"
  MVN_P2_ARGS="-Dtycho.localArtifacts=ignore -Pscala-2.12.x -Peclipse-photon $ADDITIONAL_MVN_OPTS clean verify"
else
  MVN_ARGS="$*"
